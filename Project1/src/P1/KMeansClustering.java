...
//copy "Longotor1delta.xls" into project
public class KMeansClustering 
{
	...
	public double[] normalize(double[][] d, int x) throws IOException
	{
		...
		//Formula
		// NEW VALUE = ((CURRENT VALUE - MINIMUM) / (MAXIMUM - MINIMUM)) * (TARGET MAXIMUM - TARGET MINIMUM) + TARGET MINIMUM
	...
	public double[][] manhattanDistance(double normal[], double normal1[], double normal2[], double[][] centroid, int k)
	{
		...
		for(int i = 0; i < data[x].length; i ++)
		{
			for(int j = 0; j < k; j++)
			{
				//System.out.println("centroid: " + centroid[j][0] + ", " + centroid[j][1] + ", " + centroid[j][2]);
				//System.out.println("Normal: " + normal[i] + ", " + normal1[i] + ", " + normal2[i]);
				manhattan[i][j] = Math.abs(centroid[j][0] - normal[i]) + Math.abs(centroid[j][1] - normal1[i]) + Math.abs(centroid[j][2] - normal2[i]);

			}
		}


		//System.out.println(Math.abs(centroid[0][2] - normal2[0]));
		return manhattan;
	}
	...
	public ArrayList<String>[] cluster(double[][] manhattan, double[][] centroids, int k, String[] dna, double[] norm, double[] norm1, double[] norm2, int iterations)
	{
		...
		for(int i = 0; i < dna.length; i++)
		{
			...
			for(int j = 0; j < k; j++)
			{
				//if current distance < previous distance, then set minDistance to current distance
	...
	public double[][] newCentroids(double[][] manhattan, double[][] centroids, int k, String[] dna, double[] norm, double[] norm1, double[] norm2, int iterations)
	{
		//store dna names in an arraylist
		//there will be 'k' arraylist stored in an array
	...
	public static void main(String[] args) throws IOException 
	{
		...
		//find the centroids (first ones can be random?) *use the first row of data for first centroid*
		...
		for(int i = 0; i < k; i++)
		{
			...
			//System.out.println(centroids[i][0] + " centroids[" + i + "][0]");
			//System.out.println(centroids[i][1] + " centroids[" + i + "][1]");
			//System.out.println(centroids[i][2] + " centroids[" + i + "][2]");
		...
		//ArrayList<String>[] cluster = test.cluster(manhattan, centroids, k, dna, normSCH, normRAS, normTOR, iterations);
